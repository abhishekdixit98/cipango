...
public class Compression
{
	...
	public void encodeName(Name name, Buffer buffer)
	{
		encodeName(name, buffer, false);
	}
	
	public void encodeName(Name name, Buffer buffer, boolean rdata)
	{
		while (name != null)
		{
	...
	public Name decodeName(Buffer buffer)
	{
		...
		while (true)
		{
			...
			{
				int position = buffer.getIndex() - 1;
				Name name = new Name(new String(buffer.get(size).asArray()));
