...
public class DigestAuthenticator implements SipAuthenticator
{
	...
	private void send401(SipRequest request, boolean proxyMode, boolean stale) throws IOException
	{
		...
		if (proxyMode)
		{
			...
			response.setHeader(SipHeaders.PROXY_AUTHENTICATE, authenticate.toString());
		}
		else
		{
			response = request.createResponse(SipServletResponse.SC_UNAUTHORIZED);
			response.setHeader(SipHeaders.WWW_AUTHENTICATE, authenticate.toString());
		}
		response.send();
	}
	...
	{
		Iterator<String> it = request.getFields().getValues(getAuthHeaderName(proxyMode));
		while (it.hasNext())
		{
