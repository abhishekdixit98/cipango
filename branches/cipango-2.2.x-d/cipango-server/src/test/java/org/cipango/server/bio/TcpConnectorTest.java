...
public class TcpConnectorTest
{
	...
	public void testSendHeartBeat() throws Exception
	{
		...
		_connector.getConnection(socket.getInetAddress(), socket.getLocalPort()); 
		
		for (int i = 0; i < 3; i++)
		{
			_connector.sendHeartBeat(socket.getInetAddress(), socket.getLocalPort());
			Thread.sleep(550); // Wait 550ms to ensure that the 2 PONG are not detected as a PING
			Assert.assertArrayEquals(_pingEolEol.getBytes(), ts.pop());
			assertNull(ts.pop());
			
			ts.send(_pingEol.getBytes());
		}
	...
	@Test
	public void testSendHeartBeatFlow() throws Exception
	{
		final ServerSocket socket = new ServerSocket();
	...
	@Test
	public void testReceiveHeartBeat() throws Exception
	{
		TestClientSocket ts = new TestClientSocket();
		ts.start();		
		for (int i = 0; i < 3; i++)
		{
			ts.send(_pingEolEol.getBytes());
			Thread.sleep(50);
			Assert.assertArrayEquals(_pingEol.getBytes(), ts.pop());
			assertNull(ts.pop());
